...
function app_module_methods.model(app_module, ...)
  if app_module.mapper.default then
    ...
    app_module.mapper = orm.new(app_module.mapper.table_prefix or table_prefix, app_module.mapper.conn, app_module.mapper.driver, app_module.mapper.logging)
  end
...
function _M.run(app_module, wsapi_env)
  ...
  if wsapi_handler then
    local ok, status, headers, res = xpcall(
